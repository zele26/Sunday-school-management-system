...
const TeacherExams = () => {
  ...
  const handleCreateQuiz = async (e) => {
    ...
    if (res.ok) {
      ...
      const data = await res.json().catch(() => ({}));
      setMessage(data.message || 'Error creating exam');
    }
  };
    ...
    <div className="space-y-6 font-sans">
      <Card variant="default" padding="lg">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 dark:border-slate-800 pb-4">
          <div>
            <CardTitle>የፈተና አስተዳደር (Exam Management)</CardTitle>
          ...
          >
            {showForm ? 'ሰርዝ (Cancel)' : '+ አዲስ ፈተና (+ New Exam)'}
          </button>
        </div>
            ...
            <Card key={q._id} variant="default" padding="md" className="hover:border-blue-300 transition-colors flex flex-col justify-between space-y-3">
              <div>
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Badge variant="neutral" size="xs">{q.quizType}</Badge>
                </div>

    ...
    for (let i = 1; i <= totalPages; i++) {
      buttons.push(
        <button
          key={i}
          onClick={() => setPage(i)}
          className={`px-3.5 py-2 rounded-xl text-xs font-semibold shadow-sm transition-all ${i === page
              ...
              <tbody className="divide-y divide-slate-100 text-sm text-slate-700">
                {students.map(s => (
                  <tr key={s._id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-3.5 px-4">
                      <span className={`px-2.5 py-1 text-xs font-bold rounded-lg border ${s.studentType === 'distance'
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl p-8 w-full max-w-2xl shadow-2xl border border-slate-100 max-h-[90vh] overflow-y-auto space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm bg-slate-50/50 p-5 rounded-2xl border border-slate-100">
              ...
              <div><span className="font-semibold text-slate-400 uppercase text-xs block mb-1">Email (login)</span> <span className="text-slate-800 font-medium">{selectedStudent.userId?.email || selectedStudent.email || '-'}</span></div>
              ...
              <div>
                ...
                <span className="text-slate-800 font-medium">
                  {selectedStudent.emergencyFirstName || ''} {selectedStudent.emergencyMiddleName || ''} {selectedStudent.emergencyLastName || ''}
                </span>
              </div>
              <div>
                <span className="font-semibold text-slate-400 uppercase text-xs block mb-1">Relationship</span>
                <span className="text-slate-800 font-medium">{selectedStudent.relationship || '-'}</span>
              </div>
              <div>
                <span className="font-semibold text-slate-400 uppercase text-xs block mb-1">Phone</span>
                <span className="text-slate-800 font-medium">{selectedStudent.emergencyPhone || selectedStudent.contactPhone || '-'}</span>
              </div>
              <div>
                <span className="font-semibold text-slate-400 uppercase text-xs block mb-1">Email</span>
                <span className="text-slate-800 font-medium">{selectedStudent.emergencyEmail || selectedStudent.contactEmail || '-'}</span>
              </div>
              <div className="col-span-2">
                <span className="font-semibold text-slate-400 uppercase text-xs block mb-1">Address</span>
                <span className="text-slate-800 font-medium">{selectedStudent.emergencyAddress || selectedStudent.contactAddress || '-'}</span>
              </div>

              <div className="col-span-2">
                <span className="font-semibold text-slate-400 uppercase text-xs block mb-1">Registration Date</span>
                <span className="text-slate-800 font-medium">{selectedStudent.registrationDate ? new Date(selectedStudent.registrationDate).toLocaleDateString() : '-'}</span>
              </div>
            </div>

...
const RegisterDistance = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // Send JSON – no file upload at this stage
      ...
      if (res.ok) {
        ...
        // Fetch payment info to show instructions
  ...
  if (step === 'success') {
    return (
      <div className="max-w-md mx-auto mt-20 p-6 bg-white rounded-2xl shadow text-center">
        <h2 className="text-2xl font-bold text-emerald-700 mb-4">✅ ምዝገባ ተቀባይነት አግኝቷል</h2>
        ...
        {paymentInfo && (
          <div className="mt-6 bg-slate-50 p-4 rounded-xl text-left">
            ...
            <p>
              <strong>የክፍያ መጠን:</strong> {paymentInfo.contributionAmount} ብር
            </p>
            <p>
              <strong>የትምህርት ቁሳቁስ:</strong> {paymentInfo.resourceFee} ብር
            </p>
            <p className="font-bold text-lg mt-2">
              ጠቅላላ: {paymentInfo.totalAmount} ብር
            </p>
        ...
        <p className="mt-6 text-sm text-slate-700 font-medium">
          ክፍያውን ከፈጸሙ በኋላ ደረሰኝዎን ለማስገባትና ምዝገባዎን ለማጠናቀቅ እባክዎ
          <br />
          <span className="font-bold">"ቀጥል ምዝገባ"</span> የሚለውን ይጫኑ።
        </p>

        <Link
          to="/continue-registration"
          className="mt-4 inline-block bg-blue-600 text-white px-6 py-2 rounded-xl font-semibold"
        >
          ቀጥል ምዝገባ
        </Link>
      </div>
  ...
  return (
    <div className="max-w-3xl mx-auto py-10 px-4">
      ...
      >
        <h2 className="font-semibold text-slate-600 border-b pb-2">
          የግል መረጃ
        </h2>
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-emerald-600 text-white p-3 rounded-xl font-bold hover:bg-emerald-700"
        >

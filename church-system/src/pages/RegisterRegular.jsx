...
const RegisterRegularContent = () => {
  ...
  // Reusable input and label classes
  const inputClass = "w-full px-4 py-3.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all text-slate-700 text-sm placeholder:text-slate-400";
  const labelClass = "block text-sm font-semibold text-slate-700 mb-1.5 ml-1";

  // ✅ Success screen for regular student
  if (success) {
    return (
      <div className="min-h-[80vh] flex items-center justify-center p-4 bg-slate-50/50 font-sans">
        <div className="max-w-lg w-full bg-white p-8 rounded-3xl shadow-xl border border-emerald-100 text-center animate-in zoom-in-95 duration-500">
          ...
          <p className="text-slate-500 mb-6 leading-relaxed">
            የምዝገባ ጥያቄዎ በተሳካ ሁኔታ ተልኳል። አስተዳደሩ ሲያጸድቀው መለያ ቁጥርዎን ይቀበላሉ።
          </p>
          <div className="bg-emerald-50 rounded-2xl p-5 border border-emerald-200 mb-8">
            <p className="text-xs font-semibold text-emerald-600 uppercase tracking-wider mb-1">የማመልከቻ ቁጥር (Application Number)</p>
            <p className="text-3xl font-black text-emerald-700 tracking-widest">{success?.registrationNumber}</p>
            <p className="text-sm text-emerald-600 mt-3">
              📌 ምዝገባዎ ሲጸድቅ በዚህ ቁጥር መግባት ይችላሉ።
            </p>
          </div>
          <Link to="/login" className="block w-full bg-emerald-600 hover:bg-emerald-500 text-white py-3.5 rounded-xl font-bold shadow-lg shadow-emerald-600/20 transition-all">

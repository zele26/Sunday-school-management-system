...
import { apiFetch } from '../api/apiClient';          // ← use the dynamic client
import bgImage from '../assets/Lidetachurch.jpg';
import logoImage from '../assets/SundaySchoolLogo.png'; // Ensure file exists in assets folder
...
const Login = () => {
  ...
  return (
    <div 
      className="min-h-screen w-full flex items-center justify-center p-4 md:p-6 bg-cover bg-center fixed inset-0 font-sans selection:bg-amber-500 selection:text-slate-950 overflow-y-auto"
      style={{ backgroundImage: `url(${bgImage})` }}
    >
      {/* Dynamic Ambient Background Overlay */}
      <div className="fixed inset-0 bg-slate-950/75 backdrop-blur-md"></div>

      {/* Floating Glass Top Bar Navigation */}
      <Link 
        to="/" 
        className="fixed top-5 left-5 z-30 inline-flex items-center gap-2.5 bg-slate-900/80 hover:bg-slate-900 text-slate-200 hover:text-white px-4 py-2.5 rounded-2xl text-xs font-semibold backdrop-blur-xl border border-white/10 shadow-xl shadow-slate-950/40 hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 group"
      >
        <svg className="w-4 h-4 text-amber-400 group-hover:-translate-x-1 transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="max-w-4xl w-full bg-white/95 text-slate-800 rounded-3xl shadow-[0_25px_60px_-15px_rgba(0,0,0,0.6)] overflow-hidden flex flex-col md:flex-row relative z-10 border border-white/80 backdrop-blur-2xl my-auto">

        {/* ================= LEFT BRANDING PANEL ================= */}
        <div className="md:w-5/12 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-blue-900 via-slate-950 to-blue-950 p-8 md:p-10 text-white flex flex-col justify-between items-center text-center relative overflow-hidden border-b md:border-b-0 md:border-r border-blue-800/30">
          
          {/* Subtle Background Glow Effect */}
          <div className="absolute -top-16 -left-16 w-48 h-48 bg-amber-500/10 rounded-full blur-3xl pointer-events-none"></div>
          <div className="absolute -bottom-16 -right-16 w-48 h-48 bg-blue-500/20 rounded-full blur-3xl pointer-events-none"></div>

          <div className="space-y-6 my-auto py-4 relative z-10 w-full flex flex-col items-center">
            
            {/* Logo Display Frame with Dual Halo Glow */}
            <div className="relative group cursor-pointer">
              ...
              <div className="relative w-28 h-28 md:w-32 md:w-32 p-1.5 rounded-full bg-slate-950/90 border border-white/20 shadow-2xl flex items-center justify-center overflow-hidden">
                <img 
                  src={logoImage} 
            ...
            <div className="space-y-3 max-w-xs">
              <h1 className="text-xl md:text-2xl font-extrabold tracking-wide text-white leading-snug drop-shadow-sm">
                ተክለሳዊሮስ ሰንበት ትምህርት ቤት
              </h1>
              
              {/* Decorative Accent Line */}
              <div className="flex items-center justify-center gap-2">
                <div className="h-0.5 w-6 bg-gradient-to-r from-transparent to-amber-400 rounded-full"></div>
                <div className="w-1.5 h-1.5 bg-amber-400 rounded-full"></div>
                <div className="h-0.5 w-6 bg-gradient-to-l from-transparent to-amber-400 rounded-full"></div>
              </div>

              <p className="text-[11px] md:text-xs text-amber-300/90 font-semibold tracking-widest uppercase leading-relaxed">
                የተማሪዎች፣ የመምህራን እና የአስተዳደር መድረክ
              </p>
            </div>
          </div>
          ...
          {/* Bottom Verse Footer */}
          <div className="hidden md:block text-xs text-slate-300/80 font-light italic border-t border-white/10 pt-4 w-full relative z-10">
            "ሕፃኑንም በሚሄድበት መንገድ መዝገበው"
          </div>
        </div>

        {/* ================= RIGHT FORM PANEL ================= */}
        <div className="md:w-7/12 p-8 md:p-12 bg-white/90 backdrop-blur-md flex flex-col justify-center relative">
          
          {/* Error Message Notification */}
          {error && (
            <div className="mb-6 p-4 bg-rose-50/90 border border-rose-200/80 rounded-2xl text-rose-700 text-xs flex items-start gap-3 font-medium shadow-sm animate-shake">
          ...
          <div className="max-w-md mx-auto w-full space-y-6">
            ...
            {/* Header Section */}
            <div>
              <h2 className="text-2xl md:text-3xl font-extrabold text-slate-900 tracking-tight flex items-baseline gap-2">
                <span>የአባል መግቢያ</span>
                <span className="text-amber-600 font-semibold text-sm md:text-base">(Sign In)</span>
              </h2>
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700 block tracking-wide">
                  ኢሜይል / ስልክ / የተማሪ መለያ
                </label>
                <div className="relative group">
                  ...
                  <input
                    ...
                    placeholder="example@gmail.com / 0911... / TKR-2026-..."
                    onChange={handleChange}
                    className="w-full pl-11 pr-4 py-3.5 bg-slate-50/80 border border-slate-200 rounded-2xl text-slate-900 placeholder-slate-400 text-xs md:text-sm font-medium focus:bg-white focus:border-blue-700 focus:ring-4 focus:ring-blue-700/10 shadow-inner transition-all outline-none"
                    required
                  />
                </div>
              </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full mt-3 bg-gradient-to-r from-blue-900 via-indigo-900 to-blue-950 hover:from-blue-950 hover:to-indigo-950 text-white py-3.5 rounded-2xl font-extrabold text-sm shadow-xl shadow-blue-950/25 hover:shadow-2xl hover:shadow-blue-900/40 active:scale-[0.99] transition-all duration-200 flex items-center justify-center gap-2.5 disabled:opacity-50 disabled:pointer-events-none cursor-pointer"
              >
                {loading ? (
                  <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
                ) : (
            ...
            {/* Links Footer */}
            <div className="pt-6 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs font-semibold">
              <Link 
                to="/forgot-password" 
                className="text-slate-500 hover:text-blue-700 transition-colors py-1"
              >
                ፓስዎርድ ረስተዋል?
              </Link>
              <Link 
                to="/student-register" 
                className="text-amber-600 hover:text-amber-700 font-extrabold hover:underline underline-offset-4 transition-all py-1"
              >
                አዲስ አካውንት ይክፈቱ (ይመዝገቡ)
              </Link>
            </div>

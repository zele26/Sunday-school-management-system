...
const Home = () => {
  ...
  return (
    <div className="min-h-screen bg-white font-sans antialiased text-slate-800 selection:bg-yellow-200 selection:text-blue-900">
      {/* Hero Section */}
      <section 
        className="relative min-h-[90vh] flex items-center justify-center bg-cover bg-center py-24 px-4 overflow-hidden" 
        style={{ backgroundImage: `url(${bgImage})` }}
      >
        {/* Serene Blue Overlay to match the logo and ensure text readability */}
        <div className="absolute inset-0 bg-gradient-to-b from-blue-500/80 via-blue-400/70 to-blue-600/90" />

        <div className="relative z-10 max-w-4xl mx-auto text-center text-white space-y-8">
          <span className="inline-flex items-center gap-2 bg-yellow-100/20 text-yellow-100 border border-yellow-200/40 text-sm font-semibold px-5 py-2 rounded-full backdrop-blur-md shadow-lg shadow-yellow-200/10 tracking-wide">
            <span className="w-2 h-2 rounded-full bg-yellow-300 animate-pulse"></span>
            እንኳን ደህና መጡ!
          </span>

          <h1 className="text-4xl sm:text-6xl md:text-7xl font-extrabold tracking-tight leading-tight text-white drop-shadow-lg">
            ተክለሳዊሮስ ሰንበት ትምህርት ቤት
          </h1>

          <p className="max-w-2xl mx-auto text-lg md:text-xl text-blue-50 font-medium leading-relaxed drop-shadow-md">
            የሕፃናትና ወጣቶች መንፈሳዊ ትምህርት ማዕከል — በሃይማኖትና በምግባር የታነጸ ትውልድ እንገነባለን።
          </p>

          <div className="pt-4 flex flex-col sm:flex-row justify-center items-center gap-4">
            {/* Register button - Light Yellow Accent */}
            <button
              onClick={() => setShowRegOptions(true)}
              className="w-full sm:w-auto bg-gradient-to-r from-yellow-200 to-yellow-300 hover:from-yellow-300 hover:to-yellow-400 text-blue-900 px-9 py-4 rounded-2xl font-bold shadow-xl shadow-yellow-200/20 transition-all duration-300 transform hover:-translate-y-1 active:translate-y-0"
            >
              ይመዝገቡ
            </button>

            {/* Login button - Glassmorphism with White/Blue */}
            <Link 
              to="/login" 
              className="w-full sm:w-auto bg-white/20 hover:bg-white/30 border border-white/50 backdrop-blur-xl text-white px-9 py-4 rounded-2xl font-semibold shadow-lg shadow-blue-900/20 transition-all duration-300 transform hover:-translate-y-1 active:translate-y-0"
            >
              ይግቡ
            </Link>
          </div>
      ...
      {/* Vision / Mission / Values Section */}
      <section className="py-24 max-w-6xl mx-auto px-4 bg-white">
        <div className="grid md:grid-cols-3 gap-8">
          {/* Vision */}
          <div className="bg-white p-8 rounded-3xl border border-blue-100 shadow-lg shadow-blue-50/50 hover:shadow-xl hover:border-yellow-300 transition-all duration-300 transform hover:-translate-y-1.5 text-center group">
            <div className="w-16 h-16 bg-blue-50 text-blue-500 rounded-2xl flex items-center justify-center mx-auto mb-6 group-hover:bg-blue-500 group-hover:text-white shadow-sm transition-all duration-300">
              <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
              </svg>
            </div>
            <h3 className="text-2xl font-extrabold text-blue-900 mb-3 tracking-tight">ራዕያችን</h3>
            <p className="text-slate-600 leading-relaxed font-medium">
              ማኅበረሰቡን በእግዚአብሔር ቃልና በኦርቶዶክሳዊት ተዋሕዶ ቤተ ክርስቲያን ስርዓት ማነጽ።
            </p>
          </div>
          ...
          {/* Mission */}
          <div className="bg-white p-8 rounded-3xl border border-blue-100 shadow-lg shadow-blue-50/50 hover:shadow-xl hover:border-yellow-300 transition-all duration-300 transform hover:-translate-y-1.5 text-center group">
            <div className="w-16 h-16 bg-yellow-50 text-yellow-600 rounded-2xl flex items-center justify-center mx-auto mb-6 group-hover:bg-yellow-300 group-hover:text-blue-900 shadow-sm transition-all duration-300">
              <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
              </svg>
            </div>
            <h3 className="text-2xl font-extrabold text-blue-900 mb-3 tracking-tight">ተልዕኳችን</h3>
            <p className="text-slate-600 leading-relaxed font-medium">
              ለሁሉም የዕድሜ ክልል ጥራት ያለውና ተደራሽ የሆነ የሰንበት ትምህርት አገልግሎት መስጠት።
            </p>
          </div>
          ...
          <div className="bg-white p-8 rounded-3xl border border-blue-100 shadow-lg shadow-blue-50/50 hover:shadow-xl hover:border-yellow-300 transition-all duration-300 transform hover:-translate-y-1.5 text-center group">
            ...
            <h3 className="text-2xl font-extrabold text-blue-900 mb-3 tracking-tight">እሴቶቻችን</h3>
            <p className="text-slate-600 leading-relaxed font-medium">
              እምነት፣ ፍቅር፣ አንድነት፣ ትህትና እና ታማኝነት።
            </p>
          </div>
      ...
      {/* Additional Amharic Content Section */}
      <section className="bg-gradient-to-b from-white to-blue-50 py-20 border-y border-blue-100/50">
        <div className="max-w-5xl mx-auto px-4 text-center">
          <h2 className="text-3xl sm:text-4xl font-extrabold text-blue-900 mb-4 tracking-tight">ለምን እኛን ይመርጣሉ?</h2>
          <p className="text-slate-600 max-w-2xl mx-auto mb-14 text-base font-medium">
            በሰንበት ትምህርት ቤታችን ህፃናትና ወጣቶች በመንፈሳዊ ዕውቀትና በበጎ ምግባር ታንፀው እንዲያድጉ ምቹ ሁኔታዎችን አመቻችተናል።
          </p>

          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-6 text-right">
            <div className="bg-white p-7 rounded-2xl border border-blue-100 shadow-md shadow-blue-50/50 flex items-start space-x-4 space-x-reverse transition-all hover:border-yellow-300">
              <div className="w-10 h-10 rounded-xl bg-yellow-100 text-blue-600 flex items-center justify-center font-black text-lg shrink-0 border border-yellow-200">✓</div>
              <div>
                <h4 className="font-bold text-blue-900 text-lg mb-1.5">ተሞክሮ ያላቸው መምህራን</h4>
                <p className="text-sm text-slate-600 leading-relaxed">በመንፈሳዊ ትምህርት የዳበረ ልምድ ባላቸው መምህራን የሚሰጥ ትምህርት።</p>
              </div>
            </div>
            ...
            <div className="bg-white p-7 rounded-2xl border border-blue-100 shadow-md shadow-blue-50/50 flex items-start space-x-4 space-x-reverse transition-all hover:border-yellow-300">
              <div className="w-10 h-10 rounded-xl bg-yellow-100 text-blue-600 flex items-center justify-center font-black text-lg shrink-0 border border-yellow-200">✓</div>
              <div>
                <h4 className="font-bold text-blue-900 text-lg mb-1.5">የተለያዩ የዕድሜ ክፍሎች</h4>
                <p className="text-sm text-slate-600 leading-relaxed">ከህፃናት እስከ ወጣቶች ለሁሉም ተስማሚ የሆኑ የትምህርት መርሃ ግብሮች።</p>
              </div>
            </div>
            ...
            <div className="bg-white p-7 rounded-2xl border border-blue-100 shadow-md shadow-blue-50/50 flex items-start space-x-4 space-x-reverse transition-all hover:border-yellow-300">
              <div className="w-10 h-10 rounded-xl bg-yellow-100 text-blue-600 flex items-center justify-center font-black text-lg shrink-0 border border-yellow-200">✓</div>
              <div>
                <h4 className="font-bold text-blue-900 text-lg mb-1.5">መንፈሳዊና ማህበራዊ እንቅስቃሴዎች</h4>
                <p className="text-sm text-slate-600 leading-relaxed">መዝሙር፣ ጉዞዎችና ማህበራዊ አገልግሎቶች።</p>
              </div>
            </div>
          </div>
      ...
      {showRegOptions && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-sm w-full text-center border border-blue-50 space-y-4">
            <div className="w-14 h-14 bg-blue-50 text-blue-500 rounded-2xl flex items-center justify-center mx-auto shadow-inner">
              <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" />
              </svg>
            </div>
            ...
            <div className="space-y-3 pt-2">
              <Link
                to="/register-regular"
                onClick={() => setShowRegOptions(false)}
                className="block w-full bg-blue-500 hover:bg-blue-600 text-white py-3.5 rounded-2xl font-bold shadow-md shadow-blue-500/20 transition-all"
              >
                መደበኛ (Regular)
              </Link>
              ...
              <Link
                to="/register-distance"
                onClick={() => setShowRegOptions(false)}
                className="block w-full bg-yellow-200 hover:bg-yellow-300 text-blue-900 py-3.5 rounded-2xl font-bold shadow-md shadow-yellow-200/40 transition-all"
              >
                ርቀት (Distance)
              </Link>
            </div>

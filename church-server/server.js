...
// Temporary migration endpoint – embedded directly in server.js
app.post('/api/admin/temp/run-full-education-migration', async (req, res) => {
  try {
    // Require models inside the route to avoid startup issues if missing
    ...
    for (const student of students) {
      ...
      if (!person) {
        console.warn(`Skipping student ${student.studentId} - no Person`);
        continue;
      }
